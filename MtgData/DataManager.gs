...
DataManager.prototype.ApplyCardCounts = function(new_counts) {
  for (var set_code in new_counts) {
    var localid_to_card = this.setcode_to_localid_to_card[set_code];
    var localid_to_new_counts = new_counts[set_code];
    for (var localid in localid_to_new_counts) {
      var card = localid_to_card[localid];
      var card_counts = localid_to_new_counts[localid];
